...
* Strongly typed => it is not possible for the programmer to work around the
  restrictions imposed by the type system.

* Statically typed => the type of each expression is known at compile
  time and the code can be checked for type errors by the compiler or the
  interpreter.
...
(2) Type annotations help us to clarify what function should actually do, 
    before we even start writing it.
...
> removeEverySecond s = unwords [ snd ix | ix <- zip [1..] (words s), even (fst ix)] 
...
For example, 'getLine' is an IO functions that returns a string, so its type
is:

  getLine :: IO String

On the other hand, 'putStrLn' is an IO action that prints something on the
screen, but doesn't return anything. IO functions that do some action but don't
return anything have a type 'IO ()'. The '()' here is called a "unit" and
represents an empty value (you can thing of it as a nullary tuple). So, the
type of 'putStrLn' is:

 putStrLn :: String -> IO ()

Keep in mind that IO actions can only be used within a "do" block and that you
need to unwrap their result using the "<-" operator (that is, to grab the 'a'
inside the 'IO a').
...
=== PRACTICAL HASKELL: MODULES ===============================================

Let's write a program that opens up a textual file, breaks the words into
separate strings, sorts them, removes the duplicates, and outputs to standard
output. So, the program should behave pretty much like:
...
* writing comments
* code modularization, defining modules
